...
namespace SmartCare.Application.Features.Orders.CreateOnlineOrder
{
    public class CreateOnlineOrderCommandHandler : IRequestHandler<CreateOnlineOrderFromCartAsyncCommand, Response<OrderResponseDto?>>
    {
        ...
        #endregion
        public CreateOnlineOrderCommandHandler(IConfiguration configuration, IResponseHandler responseHandler, IUnitOfWork unitOfWork, IBackgroundJobService backgroundJobService, IMapper mapper, ISqlLockManager sqlLockManager, ILogger<OrderService> logger, IEventPublisherService eventPublisherService)
        {
            ...
            _logger = logger;
            OrderExpirationTimeUntilPayment = configuration.GetValue<int>("ReservationTimes:HoursForPayment");
        ...
        private void ScheduleOrderExpiration(SmartCare.Domain.Entities.OnlineOrder order)
        {
            var delay = TimeSpan.FromMinutes(OrderExpirationTimeUntilPayment);
            _backgroundJobService.Schedule(() => RealseOrder(order), delay);
        }
        void  RealseOrder(SmartCare.Domain.Entities.OnlineOrder request)
        {
            var orderId = request.Id;
            var order = _unitOfWork.Orders.GetOrderWithDetailsByIdAsync(orderId);

            if (order is null)
                  return;

            // Idempotency: don't re-expire an already finalized order
            if (order.Result.Status is OrderStatus.Expired or OrderStatus.Cancelled or OrderStatus.Completed)
                return ;

            if (order.Result.Items is null || !order.Result.Items.Any())
                return;
            ...
            foreach (var item in order.Result.Items)
            {
                if (!item.ReservationId.HasValue)
                 ...
                 _unitOfWork.Reservations.CancelReservationAsync(
            ...
            order.Result.Status = OrderStatus.Expired;

            // Save changes through UnitOfWork
            _unitOfWork.SaveChangesAsync();
            // Push Notifaction to User
            _eventPublisherService.PublishOrderExpirationNotification(order.Result.ClientId,orderId);
        }
        private Response<OrderResponseDto?>  BuildStockErrorResponse(List<OutOfStockItemDto> items)
        {
            return _responseHandler.Success<OrderResponseDto?>(null,SystemMessages.ORDER_PLACED);
        }
    }
}
